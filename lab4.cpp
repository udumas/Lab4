...
int main(){
    ...
    /*
    This Comment is a debugging segment and should be removed before turning in
      for(int i=0; i<courseSize;i++){
      cout << courses[i] << "\n";
      for(int j=0;j<=size;j++){
        if(list[j][COURSE]==i){
          cout << j << list[j][YEAR] << " " << courses[i] << " " << list[j][STUDENTS] << "\n";
        }
      }
    }
  */
 ...
 */
    ...
    while(year!=stop){
        ...
        cout << list[size][YEAR] << " " << courses[list[size][COURSE]] << " " <<list[size][STUDENTS] << "\n next Course \n";
        size++;
...
//  int maximumYear=maxYear(list, size);
  cout << "Statistics for the years " <<  minYear(list, size) << "-" <<  maxYear(list, size) << "\n";
 ...
 */
 void totalPerCourse(int list[][ELEMENTS], string courses[], int& size, int& courseSize){
    cout << "The Number of Students per Course:\n";
    int total[1000];
    int thisTotal = 0;
    for(int i=0;i<courseSize;i++){
      thisTotal=0;
...
void minNumberOfStudents(string course[], int list[][ELEMENTS], int& courseSize, int& size){
  ...
  cout << "* The course with the MINIMUM numer of students was in " << list[location][YEAR] << " " 
...
void maxPerYears(int list[][ELEMENTS], string course[], int& size, int& courseSize){
  ...
  for(int i=min;i<=max;i++){
    ...
    cout << "                    in " << i << "the maximum number of students is in " << course[list[largest][COURSE]] << " with " 
